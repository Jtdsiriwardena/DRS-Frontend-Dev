    ...
    <div className={`${GlobalStyle.fontPoppins} flex flex-col h-screen`}>
      ...
      <div className="flex-1 p-10">
        ...
        <div className={`${GlobalStyle.caseCountBar}`}>
          <div className="flex">
            <span className={GlobalStyle.countBarTopic}>Pending count</span>
          </div>
      ...
      <table className={`${GlobalStyle.table}`} aria-labelledby="drc-table">
        <thead className={`${GlobalStyle.thead}`}>
          <tr>
            <th scope="col" className={`${GlobalStyle.tableHeader}`} aria-label="DRC Name">
              DRC Name
            </th>
            <th scope="col" className={`${GlobalStyle.tableHeader}`} aria-label="Distributed Amount (LKR)">
              Distributed Amount 
            </th>
            <th
              scope="col"
              ...
              >
                <td className={GlobalStyle.tableData}>
                  {editMode === drc.id ? (
                    <select
                      value={drc.name}
                      onChange={(e) => handleChange(e, "name", drc.id)}
                      className={`${GlobalStyle.selectBox}`}
                ...
                <td className={GlobalStyle.tableData}>
                  {editMode === drc.id ? (
                    <input
                      type="number"
                      className="border-2 border-[#0056A2] rounded-lg px-4 py-1 w-32"
                      value={drc.amount}
                      onChange={(e) =>
